...
table <- read_xlsx("counts_table.xlsx")
geneID <- select(table, 'Gene ID')
table <- table[, -1]
table <- data.frame(lapply(table, as.character), stringsAsFactors=FALSE)
table <- data.frame(lapply(table, as.numeric), stringsAsFactors=FALSE)
rownames(table) <- t(geneID)
...
write.xlsx(normalizedcounts, 'Normalized_counts.xlsx')

# 10 counts matches to CPM of 1.
plot(table[,1], normalizedcounts[,1], ylim = c(0,10), xlim = c(0,20))
abline(v = 10, h = 1)
...
group <- factor(group, levels = c("Control","halfhr","X1hr","X2hr","X3hr","X6hr","X12hr",
                                  "X24hr","X48hr"))
# Plotting
...
View(threehr)
write.xlsx(threehr, '3hr vs. Control.xlsx')
pdf('3hr.pdf')
...
View(sixhr)
write.xlsx(sixhr, '6hr vs. Control.xlsx')
pdf('6hr.pdf')
...
View(twelvehr)
write.xlsx(twelvehr, '12hr vs. Control.xlsx')
pdf('12hr.pdf')
...
View(tfhr)
write.xlsx(tfhr, '24hr vs. Control.xlsx')
pdf('24hr.pdf')
...
View(fehr)
write.xlsx(fehr, '48hr vs. Control.xlsx')
pdf('48hr.pdf')
